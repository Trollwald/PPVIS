...
class Interface:
        ...
        for xCoordinate in range(self.ocean.getSize()):
            for yCoordinate in range(self.ocean.getSize()):
                if random.randrange(4) != 0:
                    inhabitant = self.__makeInhabitant(random.choice(inhabitantType))
                    self.ocean.addInhabitant(inhabitant, [xCoordinate, yCoordinate])
    ...
    def __removeInhabitant(self):
        ...
        print("column - ", end='')
        yСoordinate = self.input(0, self.ocean.getSize() - 1)
        inhabitant = self.ocean.getCell((xСoordinate, yСoordinate))
        if isinstance(inhabitant, Inhabitants.Inhabitant):
            self.ocean.removeInhabitant(inhabitant)
